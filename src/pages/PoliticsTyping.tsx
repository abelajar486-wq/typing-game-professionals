...
import { getRandomPoliticsWords } from "@/lib/extraWordLists";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";

type Mode = "30s" | "60s" | "25w" | "50w";

const PoliticsTyping = () => {
  ...
  const reset = useCallback(() => {
    setWords(getRandomPoliticsWords(wordCount));
    setInput("");
    setWordIndex(0);
  ...
  const handleInput = (value: string) => {
    ...
    if (value.endsWith(" ")) {
      ...
      if (!isTimeBased && nextIndex >= words.length) setFinished(true);
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden py-8">
      ...
      <div className="relative z-10 flex flex-col items-center gap-6 w-full max-w-3xl px-6">
        <h1 className="font-orbitron text-3xl font-bold gradient-title tracking-[0.15em]">POLITICS TYPING</h1>
        <p className="font-mono-share text-xs text-muted-foreground tracking-widest">KOSAKATA POLITIK & PEMERINTAHAN</p>
        ...
        <input
          ...
          disabled={finished}
          className="w-full max-w-md bg-card/60 border border-border rounded-xl px-6 py-4 font-mono-share text-xl text-center text-foreground tracking-wider focus:outline-none focus:border-primary border-glow-magenta disabled:opacity-50"
          placeholder={finished ? "Selesai!" : "Mulai mengetik..."}
          autoFocus
          autoComplete="off"
          autoCapitalize="off"
          spellCheck={false}
